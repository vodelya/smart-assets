...
@register_query(LocalMongoDBConnection)
def get_count_of_all_non_transferable_assets_for_public_key_of_same_type(conn, public_key, asset_type):
    query = {
        "data.categories": "Non-Transferable",
        "data.asset_type": asset_type,
        "data.issuer": public_key
    }
    #count = conn.run(conn.collection("assets").find(query)).count()
    count = conn.run(conn.collection("assets").count_documents(query))
    return count

@register_query(LocalMongoDBConnection)
def get_count_of_assets_with_same_identifier_and_type(conn, asset_identifier, asset_type):
    query = {
        "data.asset_identifier": asset_identifier,
        "data.asset_type": asset_type
    }
    count = conn.run(conn.collection("assets").count_documents(query))  
    return count
...
@register_query(LocalMongoDBConnection)
def get_count_of_functional_assets_for_public_key(conn, public_key, asset_type):
    query = {
        "data.categories": "Functional",
