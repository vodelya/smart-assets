...
class Transaction(object):
    ...
    @classmethod
    def validate_asset(cls, bigchain, user_public_key, asset):
        """Validate if the user already possesses a Non-Transferable asset.

        Args:
            bigchain: The BigchainDB connection instance.
            user_public_key (str): The public key of the user.

        Raises:
            ValidationError: If the user already possesses a Non-Transferable asset.
        """
        asset_data = asset.get("data", {})
        asset_type = asset.get("data", {}).get("asset_type")
        asset_categories = asset.get("data", {}).get("categories")
        asset_identifier = asset_data.get("asset_identifier")
        user_public_key = asset.get("data", {}).get("public_key")

        # Check if "Functional" is in the asset_categories list
        if "Functional" in asset_categories:
            functional_category = "Functional"
        else:
            functional_category = None  # Or handle the case where it's not found

        if not asset_categories:
            raise ValueError("`asset` must contain an `categories` field")
        
        # Check if an asset with the same identifier and type already exists
        same_asset_count = bigchain.get_count_of_assets_with_same_identifier_and_type(asset_identifier, asset_type)
        if same_asset_count > 0:
            raise ValidationError("An asset with the same identifier and type already exists")

        # if asset_category == "Non-Transferable":
        #     non_transferable_asset_count  = bigchain.get_count_of_all_non_transferable_assets_for_public_key_of_same_type(user_public_key, asset_type)

        #     if non_transferable_asset_count > 0:
        #         raise ValidationError("User already possesses a Non-Transferable asset of this type")
        
        if functional_category:
            functional_asset_count = bigchain.get_count_of_functional_assets_for_public_key(user_public_key, asset_type)
            if functional_asset_count > 0:
                raise ValidationError("User already possesses a Functional asset of this type")
        return True

    @classmethod
    def validate_create(cls, tx_signers, recipients, asset, metadata, bigchain):
        ...
        for signer in tx_signers:
            cls.validate_asset(bigchain, signer, asset)
    ...
    @classmethod
    def create(cls, tx_signers, recipients, metadata=None, asset=None, bigchain=None):
        ...
        (inputs, outputs) = cls.validate_create(tx_signers, recipients, {"data":asset}, metadata, bigchain)
    ...
    def inputs_valid(self, outputs=None, bigchain=None):
        ...
        if self.operation in [
            ...
            # Validate asset for CREATE operation
            if self.operation == self.CREATE:

                for signer in self.inputs[0].owners_before:
                    Transaction.validate_asset(bigchain, signer, self.asset)
